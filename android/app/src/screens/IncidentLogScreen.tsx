...
import IncidentNewModal from '../components/IncidentNewModal'; // Import the new modal component

type IncidentLogScreenProps = {
  navigation: DrawerNavigationProp<RootStackParamList, 'IncidentLog'>;
};

const {height} = Dimensions.get('window');

const IncidentLogScreen: React.FC<IncidentLogScreenProps> = ({navigation}) => {
  const [subTab, setSubTab] = useState('All');
  const [incidentNewModalVisible, setIncidentNewModalVisible] = useState(false); // Add state for modal visibility
  ...
  const HandleAdd = () => setIncidentNewModalVisible(true);
  const handleList = () => navigation.navigate('IncidentDetails');
  const handleModuleNavigation = () => {
    navigation.navigate('ModuleScreen', {previousScreen: 'IncidentLog'});
  };

  const listData = [
    {
      category: 'Location:',
      categoryname: 'Hyderabad, Category Name',
      location: 'Department Name, Department',
      foundon: '01-04-2025',
      count: '04:40',
    },
    {
      category: 'Location:',
      categoryname: 'Eluru, Category Name',
      location: 'Department Name, Department',
      foundon: '01-04-2025',
      count: '00:00',
    },
      ...
      <View style={styles.listItemContent}>
        ...
        <View style={styles.itemDetails}>
          ...
          <Text style={styles.scheduleText}>
            <Text style={styles.scheduleLabel}>Incident: </Text>
            {item.foundon} <Text style={styles.scheduleLabel}> </Text>
            {item.count}
          </Text>
        </View>
        <AntDesign
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Incident New Modal */}
      <IncidentNewModal
        visible={incidentNewModalVisible}
        onClose={() => setIncidentNewModalVisible(false)}
