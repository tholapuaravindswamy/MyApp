...
import LFNewScreenModal from '../components/LFNewScreenModal';
...
const LostFoundScreen: React.FC<LostFoundScreenProps> = ({navigation}) => {
  const listData = [
    {
      ...
      location: 'Article Name, Article size',
      foundon: '01-04-2025',
      count: '6',
    },
    {
  ...
  const HandleAdd = () => setNewModalVisible(true);
  const HandleAllList = () => navigation.navigate('LFDetails');
  const handleModuleNavigation = () => {
    navigation.navigate('ModuleScreen', {previousScreen: 'LostFound'});
  };

  const renderListItem = ({item}: {item: any}) => (
    <TouchableOpacity style={styles.listItem} onPress={HandleAllList}>
    ...
    <Text style={styles.categoryName2}>{item.categoryname}</Text>
          ...
          <Text style={styles.scheduleText}>
            <Text style={styles.scheduleLabel}>FoundOn: </Text>
            {item.foundon} | <Text style={styles.scheduleLabel}> Count: </Text>
            {item.count}
          </Text>
        </View>
        <AntDesign
            ...
            {Object.keys(filterCounts).map(filter => (
              <TouchableOpacity
                ...
                <Text
                  ...
                  {filter === 'ToFinder' ? 'To Finder' : filter}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
